...
// <------------ ALL SECTIONS OF ROUTE WILL BE MOVED TO SEPERATE FILES LATER ------>
...
router.post('/add',upload.fields([{name:'coverImage',maxCount:1},{name:'secondImage',maxCount:1}]),(req,res)=>{
   ...
   else if(req.files.coverImage==undefined || req.files.secondImage==undefined ||  req.files.coverImage.length!=1 || req.files.secondImage.length!=1){
...
// APPROVE OR DELETE A SHOW REVIEW 
...
//  GENERIC ERROR HANDLE FOR ERRORS THAT MAY NOT BE CATCHED IN GENERAL 

router.use((err,req,res,next)=>{
    console.log(err)
    res.send("oops server side error occured ")
});
